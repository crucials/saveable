...
export default defineEventHandler<Promise<MediaInfo>>(async (event) => {
    ...
    const response = await fetch(
        API_BASE_URL + `/resolve?client_id=${clientId}&url=${trackUrl}`,
    )
    ...
    if (!response.ok) {
        if (response.status === 404) {
            throw createError({
                statusCode: 404,
                message: 'Track with that URL not found',
            })
        } else {
            throw createError({
                statusCode: 500,
                message: "Couldn't get track info",
            })
        }
    }
    ...
    if (rawTrackInfo.kind != 'track') {
        throw createError({
            statusCode: 400,
            message: 'Requested resource is not a track',
        })
    }
