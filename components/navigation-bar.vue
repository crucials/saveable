<template>
    <nav class="navigation-bar" :class="{ 'navigation-bar-expanded': expanded }" ref="navigation">
        <NuxtLink to="/" class="main-page-link">
            <img src="~~/assets/images/logo.svg" alt="Down arrow points to platform, Saveable logo"
                class="logo">
        </NuxtLink>
        ...
        <button class="expand-mobile-navigation-button" title="Expand the navigation" @click="expanded = !expanded"
            v-if="expandButtonVisible">
            <img src="~~/assets/images/expand.svg" alt="Two arrows pointing up, expand button" v-if="!expanded">
            <img src="~~/assets/images/close.svg" alt="Cross, close button" v-if="expanded"/>
        </button>
    </nav>
</template>
...
<style lang="scss" scoped>
    ...
    @media (max-width: $bottom-bar-breakpoint-width) {
        ...
        .main-page-link, .navigation-menu {
            display: none;
        }
