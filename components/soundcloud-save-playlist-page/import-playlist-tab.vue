<template>
    <NuxtLayout name="service-page">
        <template #form>
            <form class="service-form" @submit.prevent="importPlaylist">
                <Heading> Import playlist from JSON </Heading>

                <p class="white-text">
                    Creates new playlist on your account identical to playlist
                    from JSON data generated on export playlist page
                </p>
                ...
                >
                    Upload JSON file (<mark class="white-text">click</mark>
                    or
                ...
                >
                    <NuxtLink
                        to="/soundcloud-token"
                        title="Learn about SoundCloud token"
                    >
                        <img
                ...
                <div class="actions">
                    <LoadingButton :loading="loading"> Import </LoadingButton>
                ...
                <ErrorText :visible="noTokenErrorVisible">
                    You need to enter your account token so app can create
                    playlist on your account.
    ...
    <ModalWindow
        :opened="resultPlaylist.windowOpened"
        @closed="resultPlaylist.windowOpened = false"
    >
        <img
...
const DEFAULT_IMPORTING_ERROR_MESSAGE =
    'Something went wrong while importing. Make sure that ' +
...
async function importPlaylist() {
    if (uploadedFile) {
        ...
        try {
            ...
            resultPlaylist.url = (
                (await response.json()) as SoundcloudApiPlaylist
            ).permalink_url
        ...
        showErrorText(
            'You need to upload a JSON file to restore playlist from it',
        )
        loading.value = false
    }
}
