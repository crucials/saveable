    ...
    <NuxtLayout name="service-page">
        <template #form>
            <form class="service-form" @submit.prevent="exportPlaylist">
                <Heading> Export SoundCloud playlist as JSON </Heading>

                <p class="white-text">
                    Save your SoundCloud playlist for future import, for
                    example, on other account
                </p>
                ...
                <div class="actions">
                    <LoadingButton :loading="loading"> Export </LoadingButton>
...
async function exportPlaylist() {
    ...
    if (playlistLinkValue.length < 1) {
        ...
        try {
            ...
            const playlistJsonUrl = URL.createObjectURL(
                new Blob([JSON.stringify(response.data.value)]),
            )
